...
interface UseEditBookingFormArgs {
  bookings: Booking[];
...
export function useEditBookingForm({
  ...
  function openEdit(booking: Booking) {
    ...
    reset({
      ...
      beach_house_id: booking.beach_house_id ?? '',
      beach_house_booking_mode: booking.beach_house_booking_mode ?? 'overnight',
      parent_beach_house_booking_id: booking.parent_beach_house_booking_id ?? '',
      rental_type: booking.rental_type ?? '',
      pickup_location: booking.pickup_location ?? '',
      dropoff_location: booking.dropoff_location ?? '',
      rental_route_id:
        booking.booking_type === 'boat_rental' ? (booking.rental_route_id ?? '') : '',
      customer_name: booking.customer_name,
      customer_email: booking.customer_email,
      customer_phone: booking.customer_phone ?? '',
      guest_count: booking.guest_count,
      hours:
        booking.booking_type === 'boat_cruise' ||
        booking.beach_house_booking_mode === 'day_use'
          ? (booking.hours ?? undefined)
          : undefined,
      late_checkout_hours:
        booking.booking_type === 'beach_house'
          ? (booking.late_checkout_hours ?? 0)
          : 0,
      ...
      return_pickup_time:
        booking.booking_type === 'boat_rental' &&
        booking.parent_beach_house_booking_id &&
        booking.rental_type === 'round_trip'
          ? (booking.end_time ?? '')
          : '',
